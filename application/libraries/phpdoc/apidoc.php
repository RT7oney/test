...
class Apidoc {
		...
		foreach ($this->data as $group => $class) {
			foreach ($class as $className => $object) {
				...
				foreach ($object['methods'] as $method => $annotion) {
					...
					if (!is_dir($this->vender_path)) {
						mkdir($this->vender_path);
					}

					$sub_file = $this->vender_path . "{$className}/{$method}{$this->config['template_ext']}";
					if (!is_dir($this->vender_path . $className)) {
						mkdir($this->vender_path . $className);
					}
	...
	function getLineInfo($startLine, $endLine) {
		...
		if ($line <= 20) {
			return "该方法占用{$line}行, 代码块优化的很好!";
		}

		if ($line > 20 && $line < 50) {
			return "该方法占用{$line}行, 代码块优化的比较好!";
		}
	...
	function getMethodAnnotations($className) {
		foreach ($className->getMethods() as $object) {
			if ($object->name == 'get_instance' || $object->name == $className->getConstructor()->name) {
				continue;
			}
