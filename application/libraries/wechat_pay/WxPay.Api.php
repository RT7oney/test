...
class WxPayApi {
	...
	public static function unifiedOrder($inputObj, $timeOut = 6) {
		...
		if (!$inputObj->IsNotify_urlSet()) {
			$inputObj->SetNotify_url($GLOBALS['wechat_pay']['notify']); //异步通知url
		}

		$inputObj->SetAppid($GLOBALS['wechat_pay']['app_id']); //公众账号ID
		$inputObj->SetMch_id($GLOBALS['wechat_pay']['mch_id']); //商户号
	...
	private static function postXmlCurl($xml, $url, $useCert = false, $second = 30) {
		...
		if ($useCert == true) {
			...
			curl_setopt($ch, CURLOPT_SSLCERT, $GLOBALS['wechat_pay']['sslcert_path']);
			curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
			curl_setopt($ch, CURLOPT_SSLKEY, $GLOBALS['wechat_pay']['sslkey_path']);
		}
		//post提交方式
